...
function createBrandSummaryImage(item) {
  ...
  return `
    <div class="brand-mini-placeholder">
      🥩
    </div>
  `;
}
...
function renderBrandEntries(items) {
  ...
  items.forEach((item) => {
    ...
    const tags = [
      ...
      .map(
        (value) =>
          `<span class="brand-tag">${escapeHtml(value)}</span>`
      )
      .join("");
